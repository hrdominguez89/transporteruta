...
<script>
    function validarmetodo()
    {
        document.getElementById('metodo').addEventListener('change', function () {
            ...
            document.getElementById('fecharecepcion_div').style.display = this.value === 'TRANSFERENCIA' ? 'block' : 'none';
            document.getElementById('banco_div').style.display = this.value === 'TRANSFERENCIA' ? 'block' : 'none';
        });
    }
    validarmetodo();
    // si el value es cheque mostrar opcion tipoCheque 
    // si el value es transferencia mostrar fecha de emision y banco --
